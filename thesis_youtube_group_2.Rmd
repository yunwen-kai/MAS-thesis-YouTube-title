...
```{r}
youtuber <- read.csv('full_youtuber_group2_v5.csv')
...
corpus_own_stop_words_title <- tm_map(corpus_whitespace_title, removeWords, c("jenn","im","summer","mckeen","tess","christine","sarah","rae","vargas", "kelsey","simone","mel","joy","olivia","jade"))
...
text_title_all_df <- tibble(line=1:3520, text=text_title_all)
...
corpus_own_stop_words_description <- tm_map(corpus_whitespace_description, removeWords, c("jenn","im","summer","mckeen","tess","christine","sarah","rae","vargas", "kelsey","simone","mel","joy","olivia","jade"))
corpus_stem_description <- tm_map(corpus_own_stop_words_description, stemDocument)
corpus_stem_description
...
text_des_df <- tibble(line=1:3520, text=text_des_all)
...
ft_description <- findFreqTerms(frequencies_description, lowfreq = 900, highfreq = Inf)
ft_description

plot(frequencies_description,terms=findFreqTerms(frequencies_description, lowfreq=900),corThreshold=0.60)
```
...
text_title_rank1_unclean <- tibble(line=1:1175, text=text_title_rank1_unclean)
...
text_title_rank3_unclean <- tibble(line=1:1175, text=text_title_rank3_unclean)
...
text_title_df <- tibble(line=1:1175, text=text_title)
...
webshot("tmp.html", "group2_wc_1.png", delay = 5, vwidth = 1000, vheight = 1000)
...
webshot("tmp.html", "group2_wc_2.png", delay = 5, vwidth = 1000, vheight = 1000)
...
webshot("tmp.html", "group2_wc_3.png", delay = 5, vwidth = 1000, vheight = 1000)
...
saveWidget(vlog_wordcloud, "tmp.html", selfcontained = F)
webshot("tmp.html", "group2_wc_4.png", delay = 5, vwidth = 1000, vheight = 1000)
```
...
haul_token_barplot <- haul_token  %>% filter(n > 7) %>% mutate(word = reorder(word, n)) %>% ggplot(aes(word, n)) + geom_col() + xlab(NULL) + coord_flip()
haul_token_barplot
...
webshot("tmp.html", "group2_wc_5.png", delay = 5, vwidth = 1000, vheight = 1000)
...
# 151
haul_text <- left_join(haul_rank3, text_title_rank3_df, by="line") %>% select(c("text")) 
...
saveWidget(haul_wordcloud, "tmp.html", selfcontained = F)
webshot("tmp.html", "group2_wc_6.png", delay = 5, vwidth = 1000, vheight = 1000)
```
